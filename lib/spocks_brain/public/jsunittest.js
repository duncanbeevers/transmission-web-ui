...
JsUnitTest.Unit.Logger.prototype.start = function(testName) {
  ...
  var td;
  var link;
  var re = new RegExp(RegExp.escape(window.location.search) + '$');
  
  link = document.createElement('a');
  link.appendChild(document.createTextNode('solo'));
  link.setAttribute('href', window.location.href.replace(re,
    '?' + Object.toQueryString({tests: testName})));
...
JsUnitTest.testsStarted = {};
JsUnitTest.Unit.Testcase.prototype.run = function(rethrow) {
  var console_group_available = (console && console.group && console.groupEnd);
  var first_phase = !JsUnitTest.testsStarted[this.name],
      last_phase = false;
  try {
    if (first_phase && console_group_available) { console.group(this.name); }
    JsUnitTest.testsStarted[this.name] = true;
    if (first_phase) { jack.before(); }
    try {
      if (!this.isWaiting) { this.setup(); }
      this.isWaiting = false;
      jack.run(this.test.bind(this));
    } finally {
      if(!this.isWaiting) {
        last_phase = true;
        if (last_phase) { jack.after(); }
        this.teardown();
      }
    }
  } catch(e) {
    ...
    if (last_phase && console_group_available) { console.groupEnd(); }
  }
};
