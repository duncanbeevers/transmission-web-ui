...
JsUnitTest.Unit.Testcase.prototype.run = function(rethrow) {
  ...
  var log_group_end = false;
  try {
    if (!JsUnitTest.testsStarted[this.name] && console_group_available) { console.group(this.name); }
    JsUnitTest.testsStarted[this.name] = true;
    try {
      if (!this.isWaiting) { this.setup(); }
      this.isWaiting = false;
      jack(this.test.bind(this));
    } finally {
      if(!this.isWaiting) {
        log_group_end = true;
        this.teardown();
    ...
    if (log_group_end && console_group_available) { console.groupEnd(); }
  }
};
